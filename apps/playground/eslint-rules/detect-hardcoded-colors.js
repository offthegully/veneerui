 ...
 *
 * One definition of "hardcoded color" used by two enforcers:
 *   - the `veneer/no-hardcoded-colors` ESLint rule (editor + CI), and
 *   - the conformance test (`src/theme/conformance.test.ts`).
 *
 * The contract (see theme-system-implementation-plan.md, Phase 2): components
 * express every visual value through a semantic token utility (`bg-primary`,
 * `text-text-muted`, `border-border`, …) so a theme can reach all of them. A
 * hardcoded color is an island no theme can re-skin. This module spots the three
 * ways an island sneaks in.
 *
 * Plain JS (not TS) on purpose: ESLint loads rule files directly with no
 * transpile step, and Vitest imports it just as happily.
 */
