...
- cc97d2d: Add three rules that extend themeability enforcement beyond color to the axes that fail silently, and enable them in the `recommended` preset.
  ...
  Known limitations (deliberate, to avoid false positives): `cva`/`tv` variant **config objects** (classes as nested object _values_) aren't scanned, and a class-merge helper imported under a renamed local name isn't recognized.
