...
function Button({ children, variant = 'primary' }: { children: string; variant?: 'primary' | 'accent' | 'outline' | 'danger' }) {
  const base = `inline-flex items-center justify-center rounded-md px-4 py-2 text-sm font-medium shadow-sm ${motion}`;
    ...
    <div className="flex flex-col gap-8">
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="max-w-prose text-lg leading-relaxed text-text-muted">
          ...
          watch color, type, spacing, corners, borders, shadows, and motion all change together —
          no component code re-renders, only CSS variables.
        </p>
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        <article className="flex flex-col gap-2 rounded-lg border-border bg-surface-raised p-5 shadow-card [border-width:var(--border-width-default)]">
        ...
        <article className="flex flex-col gap-2 rounded-lg border-border bg-surface-raised p-5 shadow-lg [border-width:var(--border-width-default)]">
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        <div className="flex items-end rounded-lg p-5 text-text-on-primary shadow-md bg-[image:var(--gradient-primary)]">
          <span className="font-display text-2xl font-bold">Gradient surface</span>
        </div>
